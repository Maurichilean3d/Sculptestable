...
class Scene {
   ...
   * @param {Boolean} updateSelection If true, new meshes are added to selection instead of resetting it
   */
  duplicateSelectionGeneric(count, offsetXYZ, rotateXYZ, scaleXYZ, updateSelection) {
    if (!this._selectMeshes.length) return;
    ...
    // UPDATE SCENE
    if (copies.length > 0) {
      Array.prototype.push.apply(this._meshes, copies);
      this._stateManager.pushStateAdd(copies);

      if (updateSelection) {
        // Append new copies to current selection
        for(var i = 0; i < copies.length; ++i) {
            this._selectMeshes.push(copies[i]);
        }
        // Ensure we have a main mesh if it was missing
        if (!this._mesh && this._selectMeshes.length > 0) this._mesh = this._selectMeshes[0];
        
        this.getGui().updateMesh();
        this.render();
      } else {
        // Default: reset selection to the last source mesh
        this.setMesh(meshes[meshes.length - 1]);
      }
    }
  ...
  _createMeshCopy(mesh) {
    ...
    if (srcData._duplicateStartCount && srcData._duplicateStartCount.slice) {
         dstData._duplicateStartCount = srcData._duplicateStartCount.slice(); 
    }
